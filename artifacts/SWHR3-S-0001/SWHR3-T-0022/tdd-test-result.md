...
- Every selector was cross-checked by hand against the actual rendered JSX of `src/pages/signin.tsx`, `src/pages/users/create.tsx`, `src/pages/users/profile.tsx` and `src/pages/RootErrorBoundary.tsx` (all read in full during this ticket) — catching two ambiguity bugs before the first push: Playwright's `getByLabel`/`getByText` do case-insensitive **substring** matching by default (unlike Testing Library's exact-by-default), so `getByLabel("Password")` inside the New customer panel would also match "Confirm password", and `getByLabel("Street address")` would also match "Street address line 2 (optional)" — both now pass `{ exact: true }`.
- CI (`.github/workflows/ci.yml`) is what actually ran this file against a real Chromium, and it caught a **third** instance of the same bug class that manual review missed: `getByLabel("User name")` also matched the "Remember my user name" checkbox (its accessible name contains "user name" as a case-insensitive substring), failing 3 of 8 tests with a strict-mode-violation error. Fixed by adding `{ exact: true }` to every "User name" query and re-pushed; `a2a_await_ci` is tracking the corrected run. This is the clearest evidence in this ticket that CI, not local reasoning, is this file's real gate — manual review caught two of three real defects, CI caught the third.
